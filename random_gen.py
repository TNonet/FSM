...
def test_b_matrix(m=None, n=None, data_n=1, sym=False):
    while True:
        if sym:
        ...
        data_array, rows_array, cols_array, shape = SNP(_m, _n, .1, data_n=data_n, sym=sym)
...
def test_f_matrix(m=None, n=None, k=None, data_n=1, density=.1, sym=False):
    while True:
        ...
        for i in range(_k):
            ...
            data_array, rows_array, cols_array, shape = SNP(_m, _n, density, data_n=data_n, sym=sym)
            row_p, col_i = coo_to_csr(_m, len(rows_array), rows_array, cols_array)
            array_sparse = bcsr_matrix(row_p, col_i, (_m, _n))
